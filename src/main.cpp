 ...
 * We are ~100 times smaller then alqo now (2015-06-23), set minRelayTxFee only 10 times higher
 * so it's still 10 times lower comparing to alqo.
 */
...
{
    // We don't support zerocoin spends
    return false;
}
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    // Damage control
    if (sporkManager.IsSporkActive(SPORK_17_CHOKE_CONTROL_MODE)) {
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    const bool fZerocoinActive = false;
...
bool static ProcessMessage(CNode* pfrom, std::string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
		...
		// broken releases with wrong blockchain data
                if (pfrom->cleanSubVer == "/ALQO:6.5.0/" ||
        ...
        AddTimeData(pfrom->addr, nTimeOffset, 15);
    }
